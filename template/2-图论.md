...
void init(int n){
    ...
    rep(i,2,n){
        rep(j,0,i-1){
            b[i] = add(b[i], mul(C[i-1][j],mul(b[i-j-1],a[j+1])));
        }
    }
    c[0] = c[1] = 0;
    rep(i,2,n){
        rep(j,1,i-1){
            c[i] = add(c[i],mul(mul(mul(j,j),C[i-2][j-1]),P[i-1][i-j-1]));
        }
        c[i] = mul(c[i], i);
    }
    d[0] = d[1] = 0;
    rep(i,2,n){
        rep(j,0,i-1){
            d[i] = add(d[i], mul(C[i-1][j],add(mul(a[j+1],d[i-j-1]), mul(b[i-j-1],c[j+1]))));
        }
    }
}
int main(){
